...
/* Procedure d'aminicissement avec les BBDs.
   ...
   La liste de point est de type vt_vpt_amincir*.
...
#ifndef NO_PROTO
int _VT_GEDIST_THINNING( vt_image *im         /* image to be thinned */,
		      vt_vpt_amincir *liste  /* list of points which can be deleted */,
		      int *lnb               /* number of points which can be deleted */,
		      vt_amincir *par        /* parameters */,
		      int dmin               /* minimum value for end condition */ )
#else
int _VT_GEDIST_THINNING( im, liste, lnb, par, dmin )
...
#endif
{
  ...
  char message[256], *name_proc="_VT_GEDIST_THINNING";
  ...
  do {
	...
	for ( k = 0; k < 3; k++ )
	  for ( j = 0; j < 3; j++ )
	    for ( i = 0; i < 3; i++ ) {
	      if ( v[i][j][k] == VT_HASBEENDELETED ) v[i][j][k] = 0;
	    }
	...
	/* si la condition de point d'arret ne doit pas etre appliquee */
	if ( liste[l].pt.v < dmin ) {
	  liste[l].status = VT_TOBEDELETED;
	  pb[ liste[l].pt.z ][ liste[l].pt.y ][ liste[l].pt.x ] = VT_WILLBEDELETED;
	  continue;
	}
	...
	we = in;
      ...
      /*--- message ---*/
      sprintf( message,"     subiteration # %d, (min=%d) deleted points = %d", cycle+1, min, nbd );
      VT_Message( message, name_proc );
    }
    /*--- fin d'un cycle = 6 sous-cycle ---*/
    
    /*--- message ---*/
    sprintf( message," iteration, (min=%d) deleted points = %d", min, nbdel );
    VT_Message( message, name_proc );
    ...
    if ( ln <= 0 ) {
      ...
      bool_continue = 1;
      /* epais = N06; */
    } else if ( (((epais == N06) || (epais == N18)) 
		 && (epais_max == N26)) ||
		((epais == N06) && (epais_max == N18)) ) {
      bool_continue = 1;
      switch ( epais_max ) {
      case N26 :
	if ( epais == N06 ) {
