...
#ifndef NO_PROTO
int _VT_GEBDD2_THINNING( vt_image *im         /* image to be thinned */,
		      ...
		      int min_iters )
#else
int _VT_GEBDD2_THINNING( im, liste, lnb, par, min_iters )
...
#endif
{
  ...
  char message[256], *name_proc="_VT_GEBDD2_THINNING";
  ...
  do {
    if ( iterations >= min_iters ) bool_shrinking = 0;
    iterations++;
	...
	if ( liste[l].status == VT_UNDELETABLE ) continue;
    ...
    /*--- message ---*/
    sprintf( message," iteration %d, deleted points = %d", iterations, nbdel );
    VT_Message( message, name_proc );
