...
#else 
static int  VT_ComputeOneLine3D();
static int  VT_FilterImages();
#endif

#ifndef NO_PROTO
int VT_ComputeLine3D( vt_resline *res, vt_images *ims, vt_image *theim, vt_line *par )
#else
int VT_ComputeLine3D( res, ims, theim, par )
vt_resline *res;
vt_images *ims;
vt_image *theim;
vt_line *par;
...
static int VT_ComputeOneLine3D( vt_images *ims, vt_image *theim, vt_recfilters *par, vt_line *lpar )
#else
static int VT_ComputeOneLine3D( ims, theim, par, lpar )
vt_images *ims;
vt_image *theim;
vt_recfilters *par;
vt_line *lpar;
#endif
{
    ...
    */
	...
	if ( 0 ) {
	  ...
	  if ( _ComputeEigensOfSymetricSquareMatrix( hessien, val_propre, vec_propre, (int)3 ) != 1 ) continue;
	} 
...
static int VT_FilterImages( vt_images *ims, vt_image *im, vt_recfilters *par )
#else
static int VT_FilterImages( ims, im, par )
vt_images *ims;
vt_images *im;
vt_recfilters *par;
#endif
