...
#endif
{
	...
	VT_Histo( &histoXY );   histoXY.type = image->type;
	VT_Histo( &histoXZ );   histoXZ.type = image->type;
	VT_Histo( &histoZY );   histoZY.type = image->type;
	...
	VT_InitImage( &imres, image_name, histoXY.size, 1, 1, INT );
	...
	for ( n = 0; n < nb_test; n ++ ) {
	    ...
	    if ( (int)(mXY.min) == 0 ) {
		if ( VT_Image2Histo( &histoXY, &imXY, &(par.spar) ) != 1 ) {
		    VT_FreeImage( &imXY );      VT_FreeImage( &imXZ );       VT_FreeImage( &imZY );
		    VT_FreeImage( image );      VT_Free( (void**)&image );   VT_FreeImage( &imres );
		    VT_FreeHisto( &histoXY );   VT_FreeHisto( &histoXZ );    VT_FreeHisto( &histoZY ); 
	    ...
	    if ( (int)(mXZ.min) == 0 ) {
		if ( VT_Image2Histo( &histoXZ, &imXZ, &(par.spar) ) != 1 ) {
		    VT_FreeImage( &imXY );      VT_FreeImage( &imXZ );       VT_FreeImage( &imZY );
		    VT_FreeImage( image );      VT_Free( (void**)&image );   VT_FreeImage( &imres );
		    VT_FreeHisto( &histoXY );   VT_FreeHisto( &histoXZ );    VT_FreeHisto( &histoZY ); 
	    ...
	    if ( (int)(mZY.min) == 0 ) {
		if ( VT_Image2Histo( &histoZY, &imZY, &(par.spar) ) != 1 ) {
		    VT_FreeImage( &imXY );      VT_FreeImage( &imXZ );       VT_FreeImage( &imZY );
		    VT_FreeImage( image );      VT_Free( (void**)&image );   VT_FreeImage( &imres );
		    VT_FreeHisto( &histoXY );   VT_FreeHisto( &histoXZ );    VT_FreeHisto( &histoZY ); 
		...
		fprintf( stderr, "- seuil = %d\r", (int)M );
		hseuil.buf[ (int)M ] ++;
	    } else {
		n-- ;
		fprintf( stderr, "\r" );
