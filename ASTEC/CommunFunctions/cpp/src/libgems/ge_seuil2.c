...
#endif
{
	...
	/*--- on passe sur un octet en normalisant :
	      le max etant conserve dans les images MIP
	      ca ne doit pas poser de problemes         ---*/
	VT_Image( &imaux );
	VT_InitImage( &imaux, "ge_seuil2.aux", image->dim.x, image->dim.y, image->dim.z, (int)UCHAR );
	...
	if ( VT_NormaImage( image, &imaux ) != 1 ) {
	    VT_FreeImage( image );
	    VT_Free( (void**)&image );
	    VT_FreeImage( &imaux );
	...
	VT_Histo( &histo );
	histo.type = imaux.type;
	if ( VT_AllocHisto( &histo ) != 1 ) {
	    VT_FreeImage( &imaux );
	    VT_ErrorParse("unable to allocate histogram\n", 0);
	}
	/*--- calcul histogramme ---*/
	if ( VT_Image2Histo( &histo, &imaux, &(par.spar) ) != 1 ) {
	    VT_FreeImage( &imaux );
	    VT_FreeHisto( &histo );
	...
	VT_Image( &imres );
	VT_InitImage( &imres, par.names.out, histo.size, 1, 1, INT );
	if ( VT_AllocImage( &imres ) != 1 ) {
	    VT_FreeHisto( &histo );
	    VT_ErrorParse("unable to allocate output image\n", 0);
	}
	VT_Image( &imfloat );
	VT_InitImage( &imfloat, par.names.out, histo.size, 1, 1, FLOAT );
	...
	if ( VT_FitGaussienne( (r32*)imfloat.buf, histo.size, &K, &S, &M ) != 1 ) {
	    VT_FreeImage( &imres );
	    VT_FreeHisto( &histo );
