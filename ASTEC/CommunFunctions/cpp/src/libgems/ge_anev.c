...
static char *usage = "[image-in] [image-out] [-sb %f] [-ms %d] [-steps %d]\n\
\t [-inv] [-swap] [-v] [-D] [-help]";
...
#endif
{
	...
	apar.connexite = N26;
	apar.epaisseur = N26;
	if ( _VT_GEBDD2_THINNING( &imres, liste, &nb, &apar, par.min_iter_shrink ) != 1 ) {
	    VT_FreeImage( &imres );
	    VT_Free( (void**)&liste );
...
#endif
{
	...
	while ( i < argc ) {
		...
		/*--- seuils ---*/
		    ...
		    i += 1;
		    if ( i >= argc)    VT_ErrorParse( "parsing -ms...\n", 0 );
		    status = sscanf( argv[i],"%d",&(par->min_iter_shrink) );
		    if ( status <= 0 ) VT_ErrorParse( "parsing -ms...\n", 0 );
		}
		/*--- nombre de pas ---*/
