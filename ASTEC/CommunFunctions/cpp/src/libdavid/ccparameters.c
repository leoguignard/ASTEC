  ...
  case UCHAR :
    {
      ...
      for ( z=ptmin[2]; 
	    z <= ptmax[2] && z < im1->dim.z && z < im2->dim.z; 
	    z ++ )
      for ( y = ptmin[1]; 
	    y <= ptmax[1] && y < im1->dim.y && y < im2->dim.y; 
	    y ++ ) {
	i = z * im1->dim.y*im1->dim.x + y*im1->dim.x + ptmin[0];
	j = z * im2->dim.y*im2->dim.x + y*im2->dim.x + ptmin[0];
	for ( x = ptmin[0]; 
	      x <= ptmax[0] && x < im1->dim.x && x < im2->dim.x;
	      x ++, i ++, j ++ ) {
					    ...
					    vt_image *im2 )
{
  ...
  case UCHAR :
	...
	for ( z = par1->thePar[l].ptmin[2]; 
	      z <= par1->thePar[l].ptmax[2] && z < im1->dim.z && z < im2->dim.z; 
	      z ++ )
	for ( y = par1->thePar[l].ptmin[1]; 
	      y <= par1->thePar[l].ptmax[1] && y < im1->dim.y && y < im2->dim.y; 
	      y ++ ) {
	  ...
	  for ( x = par1->thePar[l].ptmin[0]; 
		x <= par1->thePar[l].ptmax[0] && x < im1->dim.x && x < im2->dim.x;
		x ++, i ++, j ++ ) {
  ...
  case UCHAR :
    {
      ...
      for ( z=minCorner[2]; z<=maxCorner[2] && z<theIm->dim.z; z ++ )
      for ( y=minCorner[1]; y<=maxCorner[1] && y<theIm->dim.y; y ++ ) {
	i = z * theIm->dim.y*theIm->dim.x + y * theIm->dim.x + minCorner[0];
	for ( x=minCorner[0]; x<=maxCorner[0] && x<theIm->dim.x; x ++, i++ ) {
					   ...
					   int *ncc )
{
  ...
  *ncc = n_labels;
  ...
  case UCHAR :
    {
      unsigned char *theBuf = (unsigned char*)theIm->buf;
      for ( i=0, z=0; z<theIm->dim.z; z++ ) 
      for ( y=0; y<theIm->dim.y; y++ ) 
      for ( x=0; x<theIm->dim.x; x++, i++ ) {
