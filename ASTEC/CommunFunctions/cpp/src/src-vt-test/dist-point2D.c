...
int main( int argc, char *argv[] )
{
  ...
  image = _VT_Inrimage( "point2D.inr" );
  ...
  VT_Image( &imres );
  VT_InitFromImage( &imres, image, "point2D-dist.inr", FLOAT );
  ...
  {
    int i,j,k;
    float ***buf = (float***)(imres.array);
    for ( k = 0; k < imres.dim.z; k ++ )
    for ( j = 0; j < imres.dim.y; j ++ )
    for ( i = 0; i < imres.dim.x; i ++ ) {
      buf[k][j][i] = sqrt( 2*2*(j-25)*(j-25) + (i-25)*(i-25) );
    }
  }
