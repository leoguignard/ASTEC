...
#endif
{
  ...
  VT_Image( &imres );
  VT_InitImage( &imres, par.names.in, 256, 256, 1, UCHAR );
  ...
  a.y = b.y = 128;
  a.z = b.z = 0;
  r2 = (a.x - b.x)*(a.x - b.x) + (a.y - b.y)*(a.y - b.y) + (a.z - b.z)*(a.z - b.z);

  theBuf = (unsigned char***)imres.array;

  ab.x = (double)b.x - (double)a.x;
  ab.y = (double)b.y - (double)a.y;
  ab.z = (double)b.z - (double)a.z;
  ...
  for ( x=0; x<imres.dim.x; x++ ) {
    ...
    if ( t1 >= 0 && 3.0 * ( t2 * r2 - (t2 + t1)*(t2 + t1) ) - t1*t1 <= 0 )
      theBuf[z][y][x] = 128;
    else 
      theBuf[z][y][x] = 0;
  }
  theBuf[a.z][a.y][a.x] = theBuf[b.z][b.y][b.x] = 255;
