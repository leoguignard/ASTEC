...
#endif
{
  ...
  D2 = sqrt( d2 ) * (1.0 + acc)  * sqrt( d2 ) * (1.0 +acc);
...

  {
    ...
    for ( x=0; x<imres.dim.x; x++ ) {
      ...
      if ( t3 <= d2 && t2 <= d2 )
	theBuf[z][y][x] = 50;
      if ( t1 >= 0 && 3.0 * ( t2 * d2 - (t2 + t1)*(t2 + t1) ) - t1*t1 <= 0 )
	theBuf[z][y][x] = 100;
    }
  }


  milieuAB.x = (a.x+b.x)/2.0;
  milieuAB.y = (a.y+b.y)/2.0;
  milieuAB.z = (a.z+b.z)/2.0;
  ...
  for ( x=0; x<imres.dim.x; x++ ) {
    ...
    cm2 = cm.x*cm.x + cm.y*cm.y + cm.z*cm.z;
    if ( cm2 <= r2 ) {
      if ( theBuf[z][y][x] == 50 )
	theBuf[z][y][x] = 75;
      continue;
    ...
    cp = (cm.x * ab.x + cm.y*ab.y + cm.z*ab.z);
    cp2 = cp*cp / d2;
    
    if ( 4.0 * oc2 * (cm2 - cp2) < (cm2-r2)*(cm2-r2) ) {
      if ( theBuf[z][y][x] == 0 )
	theBuf[z][y][x] = 25;
      if ( theBuf[z][y][x] == 50 ) {
	theBuf[z][y][x] = 150;
