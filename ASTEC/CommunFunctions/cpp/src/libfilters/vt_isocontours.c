  ...
  case UCHAR :
    {
       ...
       */
      for ( y=0; y<theIm->dim.y - 1; y++ ) {
	
	for ( x=0; x<theIm->dim.x - 1; x++ ) {
				    ...
				    int EDGE )
{
  ...
  */
  ...
  case YINF :
    if ( startx > 0 && startx < theIm->dim.x-1 ) nmaxparcours = 2;
    else nmaxparcours = 1; 
    break;
  }
  ...
  for ( nparcours = 0; nparcours < nmaxparcours; nparcours ++ ) {
    ...
    */
    ...
    case XINF : 
      break;
    case  YINF :
      if ( (startx ==  theIm->dim.x-1) || nparcours == 1 ) {
	ix = startx-1;
	iy = starty;
	CEDGE = YSUP;
      }
    ...
    */
	...
	break;
      case YSUP :
	if ( ix+1 == theIm->dim.x-1 ) finparcours = 1;
	break;
      case XINF :
	if ( iy == 0 ) finparcours = 1;
	break;
      case XSUP :
	if ( iy+1 == theIm->dim.y-1 ) finparcours = 1;
	break;
      }
      if ( finparcours == 1 ) {
				       ...
				       double threshold )
{
  ...
  for ( z=0; z<theIm->dim.z; z++ ) {
    for ( y=0; y<resIm->dim.y-1; y++ )
    for ( x=0; x<resIm->dim.x-1; x++ ) {
