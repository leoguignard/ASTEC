...
#endif
{
  ...
  if ( (theIm1->type != theIm2->type) || 
       (theIm1->type != SCHAR && theIm1->type != SSHORT) )
  ...
  */
  ...
  VT_Image( &imres );
  VT_InitImage( &imres, "", theIm1->dim.x, theIm1->dim.y, 1, UCHAR );
  ...
  case SSHORT :
    ...
    for ( a=par.a, i=1; a < 0.999; i++, a+=par.a ) {
      sprintf(output,"%s.%d",par.names.out, i );
      (void)VT_CopyName( imres.name, output );
      fprintf(stderr,"...generating %s (a=%f)\n", output, a );

      VT_ComputeSliceFromDistances( theIm1, theIm2, &imres, 0, (double)(1.0-a), (double)a );
  ...
  default :
    fprintf( stderr, "not handled in switch\n" );
  }
