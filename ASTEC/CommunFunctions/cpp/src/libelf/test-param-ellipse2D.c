/*************************************************************************
 * test-param-ellipse2D.c - test de calcul de parametres de granulometrie
 *                          sur des ellipses 2D
 *
 * $Id: test-param-ellipse2D.c,v 1.5 2001/02/13 17:50:52 greg Exp $
 *
 * DESCRIPTION: 
 *
 *
...
static char *usage = "[image-out] [-x %d] [-y %d] [-nb% d]\n\
...
#endif
{
  ...
  VT_Image( &imres );
  VT_InitImage( &imres, par.names.in, par.x, par.y, 1, USHORT );
  ...
  for ( z=0; z<par.nb; z++ ) {
    _GenerateEllipse2D( &imres, 0, 255, par.rmax, par.rmin, radius, &angle );

    switch ( par.typeComputation ) {
    default :
    case _COMPUTE_ :
    case _ESTIMATE_ :
    _ComputeComponentParameters( &imres, &thePar, (int)0, (int)255, corner, size );
    if ( _NO_VERBOSE_ == 0 )
      printf("#nb=%3d, rayons reels = (%7.3f %7.3f) - estimes = (%7.3f %7.3f)\n",
	     z, radius[0], radius[1], thePar.maxDiameter/2.0, thePar.minDiameter/2.0 );
    break;
    case _COMPARE_ :
      _SetFeretDiameterComputationToComputation();
      _ComputeComponentParameters( &imres, &thePar, (int)0, (int)255, corner, size );
      sumErrorMaxExact += fabs( thePar.maxDiameter/2.0 - radius[0] ) / radius[0];
      sumErrorMinExact += fabs( thePar.minDiameter/2.0 - radius[1] ) / radius[1];
      if ( _NO_VERBOSE_ == 0 )
	printf("#nb=%3d, rayons reels = (%7.3f %7.3f) - calcules = (%7.3f %7.3f) ",
	       z, radius[0], radius[1], thePar.maxDiameter/2.0, thePar.minDiameter/2.0 );
      _SetFeretDiameterComputationToEstimation();
      _ComputeComponentParameters( &imres, &thePar, (int)0, (int)255, corner, size );
      sumErrorMaxEstime += fabs( thePar.maxDiameter/2.0 - radius[0] ) / radius[0];
      sumErrorMinEstime += fabs( thePar.minDiameter/2.0 - radius[1] ) / radius[1];
      if ( _NO_VERBOSE_ == 0 )
	printf("- estimes = (%7.3f %7.3f)\n",
	       thePar.maxDiameter/2.0, thePar.minDiameter/2.0 );
    }
  ...
  default :
    break;
  case _COMPARE_ :
    fprintf( stderr, " Erreurs (%%) sur rayons calcules Max -> %6.3f   Min -> %6.3f\n",
	     100.0 * sumErrorMaxExact/par.nb, 100.0 * sumErrorMinExact/par.nb );
    fprintf( stderr, "                        estimes  Max -> %6.3f   Min -> %6.3f\n",
	     100.0 * sumErrorMaxEstime/par.nb, 100.0 * sumErrorMinEstime/par.nb );
  }
