...
#include <misc.h>
/*
static int _at_pyramid_level_print_blocks_after_sort_ = 0;
*/
...
#include <behavior.h>
/*
 in Pyramidal_Block_Matching
 - change a float variable in double 
 - write additional stuff
 in Block_Matching
 - change the value of rms_threshold
   (differences are for images with few slices)
 in Allocate_Blocks
 - change the numbering of blocks
 */

/*
Remark
 in Allocate_Blocks
 - change the number of allocate blocks
   (should be erroneous in the previous version)
*/
  ...
  */
  ...
  do {
    ...
    if (param->verbosef != NULL) {
      fprintf(param->verbosef,"\n" );
      fprintf(param->verbosef,"*********************************************************************\n"); 
      fprintf(param->verbosef,"                    Iteration %d       Niveau %d      Taille %dx%dx%d\n", 
	      n_iteration+1, level, 
	      theInrimage_ref->ncols, theInrimage_ref->nrows, theInrimage_ref->nplanes );
      fflush(param->verbosef);
    }
    else 
      fprintf(stderr,"                    Iteration %d       Niveau %d      Taille %dx%dx%d\r", 
	      n_iteration+1, level,
	      theInrimage_ref->ncols, theInrimage_ref->nrows, theInrimage_ref->nplanes );
    ...
    if (param->verbosef != NULL) {
      fprintf(param->verbosef, "\nNombre de blocs_flo\t=\t%d\t, actifs apres seuils\t=\t%d\n",
    ...
    */
    ...
    /* OLD BEHAVIOR
       on se fiche de savoir si ca a marche ou pas
       puisqu'en cas d'erreur, c'est l'identite qui est renvoyee
    */
    if ( scale <= 0 ) {
      if ( _verbose_ ) 
