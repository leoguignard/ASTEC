...
static int _in_lsw_computation_print_details_ = 0;
static int _in_ltsw_computation_print_details_ = 0;

void set_in_lsw_computation_print_details( int i )
{
  _in_lsw_computation_print_details_ = i;
}

void set_in_ltsw_computation_print_details( int i )
{
  _in_ltsw_computation_print_details_ = i;
}
...
#include <behavior.h>
/* this only stand for some output/error writing */
...
/* ---------------------------------------------------------------------
                  Mediane d'un tableau de valeurs.
   --------------------------------------------------------------------- */
...
double Median ( double *tab, int dim ) 
{
  ...
  */
  ...
  /* Allocation du tableau auxiliaire ou l'on recopie
     les valeurs. On evitera ainsi que le tableau initial
     soit modifie par le tri   */
  tab_aux = (double *) calloc ( dim, sizeof(double) );
  for ( i = 0; i < dim; i ++ ) 
    tab_aux [i] = tab [i];  

  /* On reordonne le tableau selon les valeurs de x */
  ...
  /* Liberation du tableau auxiliaire */
  free ( tab_aux );
...
MAD MADn ( double *tab, int dim) 
{
  ...
  /* On stocke les deviations |X-med(X)| */
  for ( i = 0 ; i < dim ; i ++ ) {
			    ...
			    PARAM *param )
{
  ...
  /* Nombre d'appariements consideres */
  N = field -> n_pairs;
  ...
  case SIMILITUDE :
      ...
      case _ScalarWeighted2DDisplacement_ :
	{
	  ...
	  xb[2] = 0.0;
	  for ( i = 0; i < N; i ++ ) {
	    /* Coordonnees barycentriques de l'appariemment courant */
...
double Trsf_Estimation ( _MATRIX *T, FIELD *field, 
			 enumTypeTransfo transfo, enumTypeEstimator estimator, PARAM *param )
{
  switch( estimator ) {
  default :
    if ( _verbose_ )
    ...
    return( -1.0 );
  case TYPE_LS :
    return( LS_Trsf_Estimation( T, field, transfo, param ) );
  case TYPE_LSSW :
    return( LSSW_Trsf_Estimation( T, field, transfo, param ) );
  }
  return( -1.0 );
}
				 ...
				 int do_some_writing )
{
  ...
  if ( param->use_lts == 0 )
    return( Trsf_Estimation( T, field, transfo, param->estimateur, param ) );
  ...
  echelle = Trsf_Estimation( T, field, transfo, param->estimateur, param );
  ...
  do {
    ...
    /* --- Tri des residus par ordre croissant --- */
    ...
    echelle = Trsf_Estimation( T, &sub_field, transfo, param->estimateur, param );
  ...
  if ( param->verbose > 0 && param->verbosef != NULL ) {
      ...
      default: break;
      case TYPE_LS :
	fprintf(stderr, "  LTS Iteration n. %d \n", iter);
	break;
      case TYPE_LSSW :
	fprintf(stderr, "  LTSW Iteration n. %d \n", iter);
	break;
      }
#else
      switch ( param->estimateur ) {
	...
	break;
      case TYPE_LSSW :
	if ( param->verbosef == stderr ) {
	  fprintf( param->verbosef, "LTSW: iteration #%2d \r", iter ); 
	}
	else {
	  fprintf( param->verbosef, "LTSW: iteration #%2d \n", iter ); 
	}
	break;
      }
#endif
  ...
  if ( refine == 1 ) {
    ...
    */
    ...
    /* Calcul des residus r_i = || y_i - R x_i - t || */
    ...
    /* --- Estimation de la variance des residus. 
       On suppose implicitement que les erreurs sont isotropes
       et que leurs composantes x, y, z sont decorrelees.
       
       On a alors:
       
       r^2 = sigma^2 chi_2 (3)
       
       Un estimateur d'echelle robuste, et naturel dans le cas du 
       LTS, est:
       
       1/h sum_i r_i^2
       
       
       Il faut lui appliquer un facteur correctif de maniere a 
       ce que 
    ...
    /* ---
       ...
       On utilise des tests du CHI2 pour decider si le residu r_i est acceptable
       ou non. Si on suppose les erreurs gaussiennes, centrees, 
       ...
       suit une loi du Chi2 a 3 degres de liberte. On rejette l'hypothese avec un
       risque de premiere espece de 0.1.
...
{
  if (  param->use_lts == 0 )
    return( Trsf_Estimation( T, field, param->transfo, param->estimateur, param ) );
...
void PrintField( FILE *f, FIELD *field )
{
  ...
  case _ScalarWeighted2DDisplacement_ :
	...
	(typeScalarWeighted2DDisplacement *)field->pairs;
      for ( i=0; i<field->n_pairs; i++ ) {
	fprintf( f, "#%2d: (%f %f) <-> (%f %f) weight = %f\n",
		 i, thePairs[i].x, thePairs[i].y,
		 thePairs[i].u, thePairs[i].v, thePairs[i].rho );
      }
    }
    break;
  case _ScalarWeighted3DDisplacement_ :
	...
	(typeScalarWeighted3DDisplacement *)field->pairs;
      for ( i=0; i<field->n_pairs; i++ ) {
	fprintf( f, "#%2d: (%f %f %f) <-> (%f %f %f) weight = %f\n",
		 i, thePairs[i].x, thePairs[i].y, thePairs[i].z,
		 thePairs[i].u, thePairs[i].v, thePairs[i].w, thePairs[i].rho );
      }
    }
    break;
  }
