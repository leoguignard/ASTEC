...
bufferType translateType( VOXELTYPE type ) 
{
  ...
  case VT_SIGNED_INT : return ( SINT );
...
int BAL_ReadImage( bal_image *image, char *name, int normalisation )
{
  ...
  if ( normalisation && type != VT_UNSIGNED_CHAR ) {

    if ( BAL_InitImage( image, name, theIm->xdim, theIm->ydim, 
			theIm->zdim, theIm->vdim, VT_UNSIGNED_CHAR ) != 1 ) {
      if ( _verbose_ )
    ...
    if ( ConvertScaleBuffer( theIm->data, translateType( type ), 
			     image->data, translateType( VT_UNSIGNED_CHAR ),
			     theIm->xdim * theIm->ydim * theIm->zdim * theIm->vdim ) != 1 ) {
      if ( _verbose_ )
	fprintf( stderr, "BAL_ReadImage: unable to convert image '%s'\n", name );
      _freeImage( theIm );
      return( -1 );
    }
...
int BAL_SmoothImage( bal_image *theIm,
		     filterType theFilter, double sigma )
{
