...
static int _at_field_computation_print_field_ = 0;


void set_at_field_computation_print_field_( int i )
{
  _at_field_computation_print_field_ = i;
}
			    ...
			    PARAM *param )
{
  ...
  measures = (double*)malloc( xyzmeas * sizeof( double ) );
  if ( measures == NULL ) {
    if ( _verbose_ ) 
  ...
  ***/
  ...
  for (i=0; i<blocs_flo->n_valid_blocks; i++) {
...
#else
	if ( 0 && param->verbose > 0 && param->verbosef != NULL ) {
    ...
    ***/
    for ( x=xyzmeas-1; x>=0; x-- ) measures[x] = measure_default;
...
#else
      ind_Buvw = a + (b + c * blocs_ref->n_blocks_y) * blocs_ref->n_blocks_x;
      /* was
  ...
  if ( param->verbose >= 1 ) {
    fprintf(stderr,"\nCalcul de champ: Il y avait %d couples\n", blocs_flo->n_valid_blocks);
    fprintf(stderr,"                 On a garde %d couples, soit %.2f%%\n",
...
#else
      ind_Buv = a + b * blocs_ref->n_blocks_y;
#endif
...
#define _DIRECTATTRIBUTES_3D_NO_TEST_ { \
	...
	sum_2 += buf[posx] * buf[posx]; \
      ...
      blocs->bloc[n].moy = sum / (double)n_pts; \
      blocs->bloc[n].diff = sum_2 - (sum * sum) / (double)n_pts; \
      blocs->bloc[n].var  = blocs->bloc[n].diff / (double)n_pts; \
    } \
    blocs->n_valid_blocks = blocs->n_blocks; \
    n_valid_blocks = blocs->n_blocks; \
  }
...
#define _ATTRIBUTES_3D_TEST_MIN_ { \
    for ( n=0; n<blocs->n_blocks; n++ ) { \
      ...
      c = blocs->bloc[n].c; \
      n_pts = 0;   moy = sum = 0.0; \
      for (i = 0; i<blocs->block_size_x; i++ ) \
      for (j = 0, posy = a + i + b*dimx; j<blocs->block_size_y; j++, posy+=dimx ) \
      for (k = 0, posx = posy + c*dimxy; k<blocs->block_size_z; k++, posx+=dimxy ) {  \
	if ( buf[posx] <= seuil_bas ) n_passive_voxels ++; \
        else { sum += buf[posx];   n_pts ++; } \
      } \
      blocs->bloc[n].inclus = ( n_passive_voxels == 0 ) ? 1 : 0; \
