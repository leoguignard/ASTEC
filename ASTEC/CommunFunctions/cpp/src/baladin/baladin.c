...
static char *usage = "-ref <reference-image> -flo <floating-image> -res <result-image>\n\
 ...
 [-norma | -rescale | -no-norma | -no-rescale] [-vsf %f %f %f] [-vsr %f %f %f]\n\
  ...
  ************************************/
  ...
  if ( p.param.verbose > 0 ) {
	...
	if ( p.param.verbosef == NULL ) {
      ...
      sprintf( strtmp, "trace-%d.txt", getpid() );
      p.param.verbosef = fopen( strtmp, "w" );
      if ( p.param.verbosef == NULL ) {
  ...
  if ( BAL_ReadImage( &Inrimage_ref, p.reference_image, p.normalisation ) != 1 ) {
  ...
  if ( BAL_ReadImage( &Inrimage_flo, p.floating_image, p.normalisation ) != 1 ) {
    BAL_FreeImage( & Inrimage_ref );
    _free_mat( &matrice_reel );
    _free_mat( &matrice_res );
  ...
  ***/
  ...
  if ( p.result_image != NULL ||  p.use_default_filenames == 1 ) {
    ...
    if ( p.inverse == 1 ) {
	 ...
	 re-read it in case of normalisation
       */
      if ( p.normalisation ) {
	BAL_FreeImage( & Inrimage_ref );
	if ( BAL_ReadImage( &Inrimage_ref, p.reference_image, 0 ) != 1 ) {
	  BAL_FreeImage( & Inrimage_ref );
	  BAL_FreeImage( & Inrimage_flo );
	  _free_mat( &matrice_reel );
	  _free_mat( &matrice_res );
	  fprintf( stderr, "%s: unable to read reference image '%s'\n", argv[0], p.reference_image );
	  exit( -1 );
	}
      }
	...
	exit( -1 );
      ...
      if ( p.normalisation ) {
	BAL_FreeImage( & Inrimage_flo );
	if ( BAL_ReadImage( &Inrimage_flo, p.floating_image, 0 ) != 1 ) {
	  BAL_FreeImage( & Inrimage_ref );
	  BAL_FreeImage( & Inrimage_flo );
	  _free_mat( &matrice_reel );
	  _free_mat( &matrice_res );
	  fprintf( stderr, "%s: unable to read floating image '%s'\n", argv[0], p.floating_image );
	  exit( -1 );
	}
      }     
...
void __Parse( int argc, char *argv[], local_par *p )
{
  ...
  for ( i=1; i<argc; i++ ) {
    ...
    /* normalisation des images d'entrees
     */
    else if ( (strcmp ( argv[i], "-norma") == 0 && argv[i][6] == '\0') 
	      || (strcmp ( argv[i], "-rescale") == 0 && argv[i][8] == '\0') ) {
      p->normalisation = 1;
    }

    else if ( (strcmp ( argv[i], "-no-norma") == 0 && argv[i][9] == '\0') 
