...
int main( int argc, char* argv[] )
{
  ...
  for ( i=1; i<argc; i++ ) {
	...
	status = sscanf( argv[i], "%d", &(p.anchorValue) );
	...
	status = sscanf( argv[i], "%d", &(p.distanceBeforeEnding) );
...

  {
	...
	resBuf[i] = 0;
      } else if ( theBuf[i] < p.anchorValue ) {
	resBuf[i] = 200;
      } else {
	resBuf[i] = 255;
      }
  ...
  if ( ThinImageWithAllParams(  bufferOut,  bufferOut, bufferDims,
				chamfer, &p ) != 1 ) {
