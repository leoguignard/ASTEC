...
static void statsPointList( FILE *f, typePointList *l, int theMin, int theMax )
{
  ...
  for ( i=theMax; i>=theMin; i-- ) {
    if ( l[i-theMin].n_points > 0 || _verbose_ >= 4 )
      fprintf( f, " list[%3d] for value %3d: %8lu points / %8lu allocated\n", 
	       i-theMin, i, l[i-theMin].n_points, l[i-theMin].n_allocated_points );
...
static int _initRegionalMaxAuxiliaryImageSubroutine( void *parameter,
						     size_t first,
						     size_t last )
{
...
static int _initRegionalMinAuxiliaryImageSubroutine( void *parameter,
						     size_t first,
						     size_t last )
{
...
static int _processListMaxSubroutine_s8( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMAX( s8 )
}
...
static int _processListMaxSubroutine_u8( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMAX( u8 )
}
...
static int _processListMaxSubroutine_s16( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMAX( s16 )
}
...
static int _processListMaxSubroutine_u16( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMAX( u16 )
}
...
static int _processListMinSubroutine_s8( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMIN( s8 )
}
...
static int _processListMinSubroutine_u8( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMIN( u8 )
}
...
static int _processListMinSubroutine_s16( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMIN( s16 )
}
...
static int _processListMinSubroutine_u16( void *parameter, size_t first, size_t last )
{
  _PROCESSLISTMIN( u16 )
}
			  ...
			  int theMin, int theMax )
{
  ...
  if ( thePointList->n_points > magic_number*getMaxChunks() ) {
    use_parallelism = 1;
  ...
  do {
	...
	return( -1 );
      ...
      aux.changes = 0;
      if ( (* _processListSubroutine)( (void *)&aux, first, last ) != 1 ) {
	if ( _verbose_ )
	  fprintf( stderr, "%s: unable to compute changes\n", proc );
	return( -1 );
      }
    ...
    if ( _verbose_ >= 3 ) {
      fprintf( stderr, "   ... %s: changes = %8d", proc, c );
      if ( _verbose_ >= 4 )
	fprintf( stderr, "\n" );
      else
	fprintf( stderr, "\r" );
...
#define _REORDERLIST( TYPE ) {                             \
	...
	break;
      ...
      if ( use_parallelism == 1 
	   && (size_t)thePointList->n_points - first > magic_number*getMaxChunks() ) {
				    ...
				    int theMin, int theMax )
{
  ...
  do {
    ...
    if ( _verbose_ >= 1 ) {
      fprintf( stderr,"%s: iteration #%3d - changes = %9d\t\t\n", proc, iteration, changes );
    }
