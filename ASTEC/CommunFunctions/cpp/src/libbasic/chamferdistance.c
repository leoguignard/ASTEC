...
static void _PositiveForwardChamferLoop_u16( u16 *theBuf,
				     int *theDim,
				     typeChamferMask *theMask );

static void _PositiveBackwardChamferLoop_u16( u16 *theBuf,
				      int *theDim,
				      typeChamferMask *theMask );
...
#define _POSITIVE_NEIGHBORS_RUN_WITH_TESTS_ {                             \
...
#define _POSITIVE_NEIGHBORS_RUN_WITHOUT_TESTS_ {                          \
    if ( theBuf[i] == 0 ) continue;                                       \
    updatedValue = theBuf[i];                                             \
    for ( n=0; n<theMask->nb; n++ ) {                                     \
        currentValue = theBuf[i+thePts[n].o] + thePts[n].inc;             \
			      ...
			      typeChamferMask *theMask )
{
  ...
  for ( i=0, z=0; z<dimz; z++ ) {
    ...
    for ( y=0; y<dimy; y++ ) {
      ...
      if ( (-xmin) >= dimx-xmax ) {
	for ( x=0; x<dimx; x++, i++ ) {
	  _POSITIVE_NEIGHBORS_RUN_WITH_TESTS_
	 }
      }
      else {
	for ( x=0; x<(-xmin); x++, i++ ) {
	...
	for ( x=(-xmin); x<dimx-xmax; x++, i++ ) {
	  if ( ! _insidey_ ) {
	    ...
	    _POSITIVE_NEIGHBORS_RUN_WITHOUT_TESTS_
	  }
	}
...
static int _ComputePositiveChamferMapInInitializedImage( void *theBuf,
					  bufferType typeBuf,
					  int *theDim,
					  typeChamferMask *theMask )
{
  ...
  case USHORT :
    _PositiveForwardChamferLoop_u16( (u16*)theBuf, theDim, &theForwardMask );
    _PositiveBackwardChamferLoop_u16( (u16*)theBuf, theDim, &theBackwardMask );
    break;
				  ...
				  int _divide_ )
{
  ...
  /* computation 
   */
  if ( _ComputePositiveChamferMapInInitializedImage( tmpBuf, USHORT, theDim, theMask ) != 1 ) {
    if ( _verbose_ )
...
static int _ComputePositiveNegativeChamferMapInInitializedImage( void *theBuf,
					  bufferType typeBuf,
					  int *theDim,
					  typeChamferMask *theMask )
{
  ...
  case SSHORT :
    _PositiveNegativeForwardChamferLoop_s16( (s16*)theBuf, theDim, &theForwardMask );
    _PositiveNegativeBackwardChamferLoop_s16( (s16*)theBuf, theDim, &theBackwardMask );
    break;
