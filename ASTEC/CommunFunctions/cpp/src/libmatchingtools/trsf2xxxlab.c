...
int main( int argc, char *argv[] )
{
  ...
  for ( i=par.first; i<=par.last; i++ ) {
	...
	return( 0 );
      ...
      parameter[i-par.first] = da;
      break;
      
    case _TX_ :
      parameter[i-par.first] =  theTrsf.mat.m[3];
      break;

    case _TY_ :
      parameter[i-par.first] =  theTrsf.mat.m[7];
      break;

    case _TZ_ :
      parameter[i-par.first] =  theTrsf.mat.m[11];
      break;

    case _A11_ :
      parameter[i-par.first] =  theTrsf.mat.m[0];
      break;

    case _A12_ :
      parameter[i-par.first] =  theTrsf.mat.m[1];
      break;

    case _A13_ :
      parameter[i-par.first] =  theTrsf.mat.m[2];
      break;

    case _A21_ :
      parameter[i-par.first] =  theTrsf.mat.m[4];
      break;

    case _A22_ :
      parameter[i-par.first] =  theTrsf.mat.m[5];
      break;

    case _A23_ :
      parameter[i-par.first] =  theTrsf.mat.m[6];
      break;

    case _A31_ :
      parameter[i-par.first] =  theTrsf.mat.m[8];
      break;

    case _A32_ :
      parameter[i-par.first] =  theTrsf.mat.m[9];
      break;

    case _A33_ :
      parameter[i-par.first] =  theTrsf.mat.m[10];
      break;

    }

    fprintf( stderr, "#%4d: %f\n", i, parameter[i-par.first] );
  }
  ...
  min = max = parameter[0];
  for ( i=par.first+1; i<=par.last; i++ ) {
    if ( min > parameter[i-par.first] ) min = parameter[i-par.first];
    if ( max < parameter[i-par.first] ) max = parameter[i-par.first];
  }
...

  {
    ...
    if ( write( fd, parameter, (par.last - par.first + 1) * sizeof(double) ) == -1 ) {
	...
	return( 0 );
      ...
      fprintf( f, "A%s=mget( %d, 'd', f);\n", _BaseName( par.resname ), (par.last - par.first + 1) );
      fprintf( f, "mclose(f);\n" );
      fprintf( f, "\n" );
      
      fprintf( f, "\n" );
      fprintf( f, "figure;\n" );
      fprintf( f, "I%s=[%d:%d];\n", _BaseName( par.resname ), par.first, par.last );
	...
	return( 0 );
      ...
      fprintf( f, "A%s=fread( f, [%d], float%lu);\n", _BaseName( par.resname ), 
	       (par.last - par.first + 1), 8*sizeof(double) );
      fprintf( f, "fclose(f);\n" );
      fprintf( f, "\n" );
