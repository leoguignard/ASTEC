...
static void VT_InitCoeff( double sigma, int length )
#else 
static void VT_InitCoeff( sigma, length )
double sigma;
int length;
#endif
{
  ...
  for ( i = 1 ; i <= _half_length; i ++ ) {
    _local_coeff[ i ] = VT_Exp( (double)(-mul * (double)i * (double)i) );
    s += _local_coeff[ i ];
  }
  /*--- normalization ---*/
  sum = 1.0 + 2.0 * s;
...
#else 
int VT_GaussianSmoothing( theIm, resIm, sigma, length )
vt_image *theIm;
vt_image *resIm;
double sigma;
int length;
