...
#else 
static void _VT_ComputeRGCoefficients();
static void _VT_ComputeRGLine();
#endif
...
int VT_RecursiveGaussian( vt_image *resIm /* result image */, 
			  ...
			  vt_filters *par /* parameters */ )
#else
int VT_RecursiveGaussian( resIm, theIm, par )
...
#endif
{
	...
	for ( z = 0; z < theIm->dim.z ; z++ )
	    ...
	    /*--- copie du tableau ---*/
	    db1 = first;
	    for ( x = 0; x < theIm->dim.x ; x++ ) *db1++ = (double)*bufIN++;
	    ...
	    /*--- copie de la ligne ---*/
	    db1 = first;
	    for ( x = 0; x < theIm->dim.x ; x++ ) *bufOUT++ = (float)*db1++;
	}	
    }
...
static void _VT_ComputeRGCoefficients( _vt_RGcoeff *c, double s )
#else
static void _VT_ComputeRGCoefficients( c, s )
_vt_RGcoeff *c;
double s;
#endif
{
    ...
    /*--- coefficients ---*/
    if ( s <= 2.5 ) 
	q = 3.97156 - 4.14554 * VT_Sqrt( (double)( 1.0 - 0.26891 * s ) );
    else 
	q = 0.98711 * s - 0.96330;
...
#else
static void _VT_ComputeRGLine( in, out, c, dim, order )
double *in;
double *out;
_vt_RGcoeff *c;
int dim;
...
#else
int VT_LaplacianWithFD( resIm, theIm, dim )
vt_image *resIm; /* result image */ 
vt_image *theIm; /* input image */
int dim; /* dimension of the computation */ 
#endif
{
