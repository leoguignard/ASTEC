...
#else
static void _VT_FreePtList();
static int  _VT_AddPtList();

static void _VT_FreeVPtList();
static int  _VT_AddVPtList();
static void _VT_SortVPtList();
static int  _VT_CompareVPts();
#endif

#ifndef NO_PROTO
vt_pt_amincir *_VT_ThinPtList( vt_image *image, int dim, int *nb )
#else
vt_pt_amincir *_VT_ThinPtList( image, dim, nb )
vt_image *image;
int dim;
int *nb;
#endif
...
static void _VT_FreePtList( vt_pt_amincir_list *l )
#else
static void _VT_FreePtList( l )
vt_pt_amincir_list *l;
#endif
{
	int i, nb;
	if ( l->n == 0 ) return;
...
#else
vt_vpt_amincir *_VT_ThinVPtList( image, value, dim, nb )
vt_image *image;
vt_image *value;
int dim;
int *nb;
...
static void _VT_FreeVPtList( vt_vpt_amincir_list *l )
#else
static void _VT_FreeVPtList( l )
vt_vpt_amincir_list *l;
#endif
{
	int i, nb;
	if ( l->n == 0 ) return;
...
#else
static void _VT_SortVPtList( list, left, right )
vt_vpt_amincir* list;
int left;
int right;
#endif
...
static int _VT_CompareVPts( vt_vpt_amincir* pt1, vt_vpt_amincir* pt2 )
#else
static int _VT_CompareVPts( pt1, pt2 )
vt_vpt_amincir* pt1;
vt_vpt_amincir* pt2;
#endif
{
