...
static void _VT_AutreX( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dx );
static void _VT_VectSeuil( vt_image *resIm, vt_image *theIm, vt_distance *par );
#else 
static void _VT_AutreZ();
static void _VT_AutreY();
static void _VT_AutreX();
static void _VT_VectSeuil();
#endif
...
#else
int VT_DanVecteurPPP_SS( theIm, resVx, resVy, resVz, par )
	...
	*(vly + i) = *(vlz + i) = *(vlx + i);
	...
	for ( y = 0; y < dimy; y++ ) {
		...
		for ( x = 1; x < dimx; x++ ) {
			...
			_VT_AutreX( pvlx, pvly, pvlz, x, y, z, dim, (int)(-1) );
		    }
		}
	    } else {
	...
	for ( x = dimx-2; x >= 0; x-- ) {
		...
		_VT_AutreX( pvlx, pvly, pvlz, x, dim[1], z, dim, (int)(1) );
	}
	/*--- y < dimy - 1 = dim[1] ---*/
	for ( y = dimy-2; y >= 0; y-- ) {
	    /*--- x = dimx - 1 = dim[0] ---*/
	...
	for ( x = 1; x < dimx; x++ ) {
		...
		_VT_AutreX( pvlx, pvly, pvlz, x, (int)0, z, dim, (int)(-1) );
	}
	/*--- y > 0 ---*/
	for ( y = 1; y < dimy; y++ ) {
	    /*--- x = 0 ---*/
...
static void _VT_AutreZ( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dz )
#else
static void _VT_AutreZ( vx, vy, vz, x, y, z, dim, dz )
s16 ***vx;
s16 ***vy;
s16 ***vz;
int x;
...
static void _VT_AutreY( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dy )
#else
static void _VT_AutreY( vx, vy, vz, x, y, z, dim, dy )
s16 ***vx;
s16 ***vy;
s16 ***vz;
int x;
...
static void _VT_AutreX( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dx )
#else
static void _VT_AutreX( vx, vy, vz, x, y, z, dim, dx )
s16 ***vx;
s16 ***vy;
s16 ***vz;
int x;
...
static void _VT_VectSeuil( vt_image *resIm, vt_image *theIm, vt_distance *par )
#else
static void _VT_VectSeuil( resIm, theIm, par )
vt_image *resIm;
vt_image *theIm;
vt_distance *par;
#endif
