...
#define _MESURE_2D_SAD_FLO_INC_REF_INC {                       \
    ...
    for (j = 0, fposy = b*fdx, rposy = v*rdx; j < blockdim->y; j++, fposy += fdx, rposy += rdx) \
      for (i = 0, fpos = fposy + a, rpos = rposy + u; i < blockdim->x; i++, fpos++, rpos++ ) { \
        sum_diff += fabs(flo_buf[ fpos ]-ref_buf[ rpos ]); \
	npts ++;                                            \
      }                                                       \
    return( sum_diff / npts ); \
  }
...
int _ComputeBlockAttributes2D ( void *parameter,
				size_t first,
				size_t last )
{
  char *proc = "_ComputeBlockAttributes2D";
  _BlockAttributesParam *p = ( _BlockAttributesParam*)parameter;
  ...
  if ( p->blocks->border.x == 0 && p->blocks->border.y == 0 ) 
    return( _ComputeBlockAttributesWithNoBorders2D( p->inrimage, p->blocks, first, last ) );

  return( _ComputeBlockAttributesWithBorders2D( p->inrimage, p->blocks, first, last ) );
}
...
int _ComputeBlockAttributes3D ( void *parameter,
				size_t first,
				size_t last )
{
  ...
  if ( p->blocks->border.x < 0 || p->blocks->border.y < 0 || p->blocks->border.z < 0 ) {
    if ( _verbose_ ) 
      fprintf( stderr, "%s: negative values of blocks borders\n", proc );
    return( 0 );
  }
  ...
  if ( p->blocks->border.x == 0 && p->blocks->border.y == 0 && p->blocks->border.z == 0 ) {
    return( _ComputeBlockAttributesWithNoBorders3D( p->inrimage, p->blocks, first, last ) );
  }
  
  return( _ComputeBlockAttributesWithBorders3D( p->inrimage, p->blocks, first, last ) );
}
