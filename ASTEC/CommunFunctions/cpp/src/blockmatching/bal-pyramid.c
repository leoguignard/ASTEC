...
int BAL_BuildPyramidImage( bal_image *theIm, 
			   char *output_prefix_image,
			   char *output_suffix_image,
			   char *output_prefix_trsf,
			   ...
			   int pyramid_gaussian_filtering )
{
  ...
  for ( l=highest_level; l>=lowest_level; l-- ) {
    ...
    /* image name
     */
    if ( output_prefix_image[0] == '\0' ) {
      if ( output_suffix_image[0] == '\0' ) {
	sprintf( name, "image_atlevel%02d.hdr", l );
      }
      else {
	sprintf( name, "image_atlevel%02d.%s", l, output_suffix_image );
      }
    }
    else {
      if ( output_suffix_image[0] == '\0' ) {
	sprintf( name, "%s_atlevel%02d.hdr", output_prefix_image, l );
      }
      else {
	sprintf( name, "%s_atlevel%02d.%s", output_prefix_image, l, output_suffix_image );
      }
    }
    ...
    if ( BAL_WriteImage( &resIm, name ) != 1 ) {
      ...
      BAL_FreeTransformation( &subsampling_trsf );
      if ( _verbose_ )
	fprintf( stderr, "%s: unable to write image at level %d\n", proc, l );
      MAXIMAL_DIMENSION = maximal_dimension;
      return( -1 );
    }
    ...
    /* transformation name
     */
    if ( output_prefix_trsf[0] == '\0' ) {
      sprintf( name, "transformation_atlevel%02d.trsf", l );
    }
    else {
      sprintf( name, "%s_atlevel%02d.trsf", output_prefix_trsf, l );
    }
