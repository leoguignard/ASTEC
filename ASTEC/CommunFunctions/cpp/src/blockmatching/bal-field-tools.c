...
static int _ComputePairingField3D ( void *parameter,
				    size_t first,
				    size_t last )
{
  ...
  ***/
  ...
  for (i=first; i<=last; i++) {
    ...
    ***/
    ...
    if ( maxu >= blocs_ref->blocksarraydim.x ) maxu = blocs_ref->blocksarraydim.x - 1;
    if ( maxv >= blocs_ref->blocksarraydim.y ) maxv = blocs_ref->blocksarraydim.y - 1;
    if ( maxw >= blocs_ref->blocksarraydim.z ) maxw = blocs_ref->blocksarraydim.z - 1;
...
static int _ComputePairingField2D ( void *parameter,
				    size_t first,
				    size_t last )
{
...
***/
int BAL_ComputePairingFieldFromFloToRef( FIELD *field,
