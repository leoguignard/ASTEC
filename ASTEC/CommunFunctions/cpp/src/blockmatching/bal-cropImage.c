	      ...
	      )
{
  ...
  */
  ...
  if ( template_image_name != NULL ) {
    ...
    theLeftCorner[1] = origin.y;
    theLeftCorner[2] = origin.z;

    theRightCorner[0] = origin.x + template.ncols;
    theRightCorner[1] = origin.y + template.nrows;
    theRightCorner[2] = origin.z + template.nplanes;
    ...
    if ( theRightCorner[0] > theim.ncols ) theRightCorner[0] = theim.ncols;
    if ( theRightCorner[1] > theim.nrows ) theRightCorner[1] = theim.nrows;
    if ( theRightCorner[2] > theim.nplanes ) theRightCorner[2] = theim.nplanes;
   ...
   */
    ...
    theRightCorner[0] = origin.x + dim.x;
    theRightCorner[1] = origin.y + dim.y;
    ...
    if ( dim.z > 0 ) {
      ...
      theLeftCorner[2] = origin.z;
      theRightCorner[2] = origin.z + dim.z;
   ...
   */
  else if ( slice.z >= 0 && slice.z < theim.nplanes ) {
    if ( BAL_InitAllocImage( &resim, (char*)NULL,
  ...
  else if ( slice.y >= 0 && slice.y < theim.nrows ) {
    if ( BAL_InitAllocImage( &resim, (char*)NULL,
    ...
    theTrsf.mat.m[6] = -1.0;
    theTrsf.mat.m[9] = 1.0;
    theTrsf.mat.m[7] = slice.y;

    resDim[0] = theim.ncols;
    resDim[1] = 1;
    resDim[2] = theim.nplanes;

    theLeftCorner[1] = slice.y;
  }

  else if ( slice.x >= 0 && slice.x < theim.ncols ) {
    if ( BAL_InitAllocImage( &resim, (char*)NULL,
