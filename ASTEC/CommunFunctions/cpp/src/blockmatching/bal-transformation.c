			    ...
			    bal_transformation *resTrsf )
{
  ...
  case VECTORFIELD_2D :
    ...
    break;
    /* end ofVECTORFIELD_2D case for theTrsf
     */
...
{
  char *proc = "BAL_SetTransformationToIdetity";
  size_t i, v;
  float *buf;
...
static void _PrintVectorFieldStatistics( FILE *f, bal_image *vx, bal_image *vy, bal_image *vz )
{
  ...
  case FLOAT :
	...
	fprintf( f, "vmin = [%f,%f,%f], vmax = [%f,%f,%f], vmean = [%f,%f,%f]\n",
	...
	fprintf( f, "vmin = [%f,%f], vmax = [%f,%f], vmean = [%f,%f]\n",
		 vmin[0], vmin[1],
		 vmax[0], vmax[1],
		 vm[0]/v, vm[1]/v );
      }
