			   ...
			   bal_blockmatching_pyramidal_param *p )
{
  ...
  /* blocks geometry
   */
  if ( p->block_dim.x > reference->ncols )
    p->block_dim.x = reference->ncols;
  if ( p->block_dim.y > reference->nrows )
    p->block_dim.y = reference->nrows;
  if ( p->block_dim.z > reference->nplanes )
    p->block_dim.z = reference->nplanes;

  if ( p->block_dim.x + 2*p->block_border.x > reference->ncols )
    p->block_border.x = (reference->ncols - p->block_dim.x) / 2;
  if ( p->block_dim.y + 2*p->block_border.y > reference->nrows )
    p->block_border.y = (reference->nrows - p->block_dim.y) / 2;
  if ( p->block_dim.z + 2*p->block_border.z > reference->nplanes )
    p->block_border.z = (reference->nplanes - p->block_dim.z) / 2;

  /* pairing parameters
   */
  if ( 2*p->half_neighborhood_size.x + 1 > reference->ncols )
    p->half_neighborhood_size.x = (reference->ncols - 1) / 2;
  if ( 2*p->half_neighborhood_size.y + 1 > reference->nrows )
    p->half_neighborhood_size.y = (reference->nrows - 1) / 2;
  if ( 2*p->half_neighborhood_size.z + 1 > reference->nplanes )
