...
void BAL_SetVerboseInBalMatching( int v )
{
  _verbose_ = v;
}

void BAL_IncrementVerboseInBalMatching(  )
{
  _verbose_ ++;
}

void BAL_DecrementVerboseInBalMatching(  )
{
  _verbose_ --;
  if ( _verbose_ < 0 ) _verbose_ = 0;
}

void BAL_SetDebugInBalMatching( int v )
{
  _debug_ = v;
}

void BAL_IncrementDebugInBalMatching(  )
{
  _debug_ ++;
}

void BAL_DecrementDebugInBalMatching(  )
{
  _debug_ --;
  if ( _debug_ < 0 ) _debug_ = 0;
}

void BAL_SetTraceInBalMatching( int v )
{
  _trace_ = v;
}

void BAL_IncrementTraceInBalMatching(  )
{
  _trace_ ++;
}

void BAL_DecrementTraceInBalMatching(  )
{
  _trace_ --;
...
int BAL_PyramidalBlockMatching( bal_image *theInrimage_ref, /* reference image 
							     */
				...
				bal_blockmatching_pyramidal_param *theParam )
{
  ...
  bal_transformation *transformation = (bal_transformation *)NULL, level_trsf;
  ...
  BAL_AdjustBlockMatchingPyramidalParameters( theInrimage_ref, theInrimage_flo, &param );
  ...
  for ( l=pyramid_highest_level; l>=param.pyramid_lowest_level; l-- ) {
     ...
     */
    if ( param.verbosef != NULL ) {
      _PrintPyramidLevel( param.verbosef, &(pyramid_level[l]), (_verbose_>1) ); 
    }
    ...
    */
	...
	return( -1 );
      ...
      */
      ...
      if ( BAL_AllocTransformation( &level_trsf, theTr->type, &subsampled_ref ) != 1 ) {
	if ( _verbose_ ) 
	...
	return( -1 );
      ...
      */
      if ( BAL_TransformationComposition( &level_trsf, theTr, &subsampling_trsf ) != 1 ) {
	if ( _verbose_ ) 
	...
	BAL_PrintTransformation( stderr, theTr, "theTr" );
    ...
    if ( _debug_ >= 2 ) {
      ...
      if ( theInit != (bal_transformation*)NULL ) 
	BAL_PrintTransformation( stderr, theInit, "    initial transformation" );
      else 
    ...
    if ( BAL_BlockMatching( Inrimage_ref, Inrimage_flo, transformation, theInit,
	...
	BAL_PrintTransformation( stderr, &subsampling_trsf, "=== sur-echantillonage" );
       ...
       */
      if ( BAL_TransformationComposition( theTr, &level_trsf, &subsampling_trsf ) != 1 ) {
	if ( _verbose_ ) 
    ...
    if ( _debug_ >= 2 ) {
      ...
      BAL_PrintTransformation( stderr, theTr, "    transformation apres recalage" );
      fprintf( stderr, " ======================\n" );
      fprintf( stderr, "\n" );
    }
  ...
  ************************************************************/
  for ( n_iteration = 0, rms_stop = 0;
	...
	fprintf( stderr, "%s: transformation composition with the initial transformation\n", proc );
      ...
      if ( BAL_AllocTransformationComposition( &resamplingTrsf, theInit, theTr, theInrimage_ref ) != 1 ) {
	if ( _verbose_ )
	...
	return( -1 );
      ...
      if ( BAL_TransformationComposition( &resamplingTrsf, theInit, theTr ) != 1 ) {
	if ( _verbose_ )
	...
	fprintf( stderr, "%s: unable to resample floating image at iteration #%d\n", proc, n_iteration+1 );
      if ( theInit != (bal_transformation*)NULL ) BAL_FreeTransformation( &resamplingTrsf );
      BAL_FreeTransformation( &incTrsf );
      BAL_FreeBlocksAndField( &field, &blocs_flo, &blocs_ref );
      BAL_FreeImage( & Inrimage_flo_sub );
      return( -1 );
   ...
   */ 
    ...
    */
    ...
    if ( BAL_ComputePairingFieldFromFloToRef( &field, 
