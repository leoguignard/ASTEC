...
static char *usage = "[image-in] [-mask %s]\n\
...
int main( int argc, char *argv[] )
{
  ...
  if ( par.names.ext[0] != '\0' ) {
	...
	case DOUBLE :
	  {
	    double ***theBuf = (double***)image->array;
    ...
    for ( i=0; i<256; i++ ) {
      if ( snb[ i ] > 0 ) {
	fprintf( stdout, "label= %3d ", i );
	fprintf( stdout, "sum(jac)= %11.2f ", sum[ i ] );
	fprintf( stdout, "n(pts)= %8d ", snb[ i ] );
	fprintf( stdout, "sum/n= %7.5f ", sum[ i ]/(double)snb[ i ] );
	fprintf( stdout, "n/sum= %7.5f ", (double)snb[ i ]/sum[ i ] );
	fprintf( stdout, "\n" );      }
    }
