...
static char *usage = "[image-in] [image-ext] [labels-file]\n\
\t [image-in-out] [image-ext-out] [labels-file-table]\n\
...
int main( int argc, char *argv[] )
{
  ...
  _MATRIX mat;
  int i, t, val, ind, oldval;
  unsigned char *bufU8_in, *inU8;
  unsigned short int *bufU16_in, *inU16;

  unsigned char *bufU8_ext, *extU8;
  unsigned short int *bufU16_ext, *extU16;
  ...
  oldval=0;
  for (i=0 ; i<image_in->dim.x*image_in->dim.y*image_in->dim.z ; i++ ) {
      switch (image_in->type)  {
      case UCHAR:
      case SCHAR:
          val=(int)inU8[i];
  ...
  for (i=0 ; i<image_ext->dim.x*image_ext->dim.y*image_ext->dim.z ; i++ ) {
      switch (image_ext->type)  {
      case UCHAR:
      case SCHAR:
          val=(int)extU8[i];
