...
//  int flag_3D = 1;
  ...
  unsigned short int ***imgarray;

  unsigned char ***imgarrayu8;
  ...
  {
      VT_Error("this functionnality has not been implemented yet\n",program);
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      VT_FreeImage( &imres );
      return( 0 );
  ...
  return( 1 ); // returns the number of deleted labels
}
