...
#endif
{
	...
	if ( 0 ) {
	  ...
	  if ( par.type == TYPE_UNKNOWN || par.type == image->type ) {
	    ...
	    if ( par.type != TYPE_UNKNOWN ) imtmp.type = par.type;
	...
	/* some tests on the returned value
	 */
	...
	if ( par.type != TYPE_UNKNOWN ) {
	  if ( par.type != imtmp.type ) {
	    VT_InitFromImage( &imout, &imtmp, par.names.out, par.type );
	    if ( VT_AllocImage( &imout ) != 1 ) {
	      VT_FreeImage( &imtmp );
	      VT_ErrorParse("unable to allocate image to be written (1)\n", 0);
	    }
	    if ( VT_CopyImage( &imtmp, &imout ) != 1 ) {
	      VT_FreeImage( &imout );
	      VT_FreeImage( &imtmp );
	      VT_ErrorParse("unable to copy output image (1)\n", 0);
	    }
	    VT_FreeImage( &imtmp );
	    imres = &imout;
	  }
	 ...
	 */
	  ...
	  default :
	    VT_FreeImage( &imtmp );
	    VT_ErrorParse( "output type undefined or not handled\n", 0 );
	    break;
	  case VT_BINAIRE :
	    VT_InitFromImage( &imout, &imtmp, par.names.out, UCHAR );
	    if ( VT_AllocImage( &imout ) != 1 ) {
	      VT_FreeImage( &imtmp );
	      VT_ErrorParse("unable to allocate image to be written\n", 0);
	    }
	    if ( VT_Threshold( &imtmp, &imout, (float)1.0 ) != 1 ) {
	      VT_FreeImage( &imout );
	      VT_FreeImage( &imtmp );
	      VT_ErrorParse("unable to threshold output image\n", 0);
	    }
	    VT_FreeImage( &imtmp );
	    imres = &imout;
	    break;
	  ...
	  case VT_SIZE :
	    if ( VT_3m( &imtmp, (vt_image*)NULL, &m ) == -1 ) {
	      VT_FreeImage( &imtmp );
	      VT_ErrorParse("unable to compute minimum, mean and maximum", 0);
	    }
	    ...
	    if ( par.type != imtmp.type ) {
	      ...
	      if ( VT_AllocImage( &imout ) != 1 ) {
		VT_FreeImage( &imtmp );
		VT_ErrorParse("unable to allocate image to be written (2)\n", 0);
	      }
	      if ( VT_CopyImage( &imtmp, &imout ) != 1 ) {
		VT_FreeImage( &imout );
		VT_FreeImage( &imtmp );
		VT_ErrorParse("unable to copy output image (2)\n", 0);
	      }
	      VT_FreeImage( &imtmp );
	      imres = &imout;
	    }
