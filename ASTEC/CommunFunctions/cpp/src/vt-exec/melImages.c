...
int main( int argc, char *argv[] )
{
  ...
  */
  ...
  if ( par.names.ext[0] != '\0' ) {
    ...
    if ( theRightCorner[0] > image->dim.x ) theRightCorner[0] = image->dim.x;
    if ( theRightCorner[1] > image->dim.y ) theRightCorner[1] = image->dim.y;
    if ( theRightCorner[2] > image->dim.z ) theRightCorner[2] = image->dim.z;
   ...
   */
  else if ( par.slice.z >= 0 && par.slice.z < image->dim.z ) {
  ...
  else if ( par.slice.y >= 0 && par.slice.y < image->dim.y ) {
  ...
  else if ( par.slice.x >= 0 && par.slice.x < image->dim.x ) {
  ...
  */
  ...
  if ( resRightCorner[0] > imres->dim.x ) 
    winDim[0] -= resRightCorner[0]-imres->dim.x;
  if ( resRightCorner[1] > imres->dim.y ) 
    winDim[1] -= resRightCorner[1]-imres->dim.y;
  if ( resRightCorner[2] > imres->dim.z ) 
  ...
  if ( winDim[0] + par.out_origin.x > imres->dim.x
       || winDim[1] + par.out_origin.y > imres->dim.y
       || winDim[2] + par.out_origin.z > imres->dim.z ) {
      if ( _VT_VERBOSE_ ) {
