...
static int _ChangeDistanceAtBordersAlongXY();
static int _ComputeChamferWeights();
{
...
#endif
{
  ...
  case MODE_ISOTROPIC :
    ...
    */
    ...
    if ( resdim.x == image->dim.x ) par.yz = 0;
    if ( resdim.y == image->dim.y ) par.xz = 0;
    if ( resdim.z == image->dim.z ) par.xy = 0;
...
static int _ChangeDistanceAtBordersAlongXY( vt_image *imdist )
{
  ...
  case SSHORT :
    {
      ...
      for ( z=0; z<imdist->dim.z; z ++ ) {
	min[z] = max[z] = theBuf[z][0][0];
	for ( y=0; y<imdist->dim.y; y ++ )
        for ( x=0; x<imdist->dim.x; x ++ ) {
  ...
  for ( z=0; z<imdist->dim.z; z ++ ) {
    if ( min[z] == max[z] ) {
      val[z] = min[z];
      if ((z>0 && min[z-1] != max[z-1]) && (z<imdist->dim.z-1 && min[z+1] != max[z+1])) {
	val[z] = (-min[z-1] -min[z+1])/2;
      }
      else if (z>0 && min[z-1] != max[z-1]) {
	val[z] = (-min[z-1]);
      }
      else if (z<imdist->dim.z-1 && min[z+1] != max[z+1]) {
	val[z] = (-min[z+1]);
      }
    }
  }
