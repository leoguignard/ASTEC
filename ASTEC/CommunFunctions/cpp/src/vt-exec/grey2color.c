...
static int _VT_Grey2Color();
static int _VT_Color2Grey();
#endif
  ...
  */
  ...
  case VT_COLOR2GREY :
    if ( image->type != UCHAR ) {
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      VT_ErrorParse("bad type for input image\n", 0);
    }
    /*--- verifications ---*/
    if ( image->dim.v == 3 ) {
      par.type_image = VT_VECTORIELLE;
    ...
    if ( _VT_Color2Grey( &imres, image, &(par.coeff_c2g) ) != 1 ) {
      ...
      VT_Free( (void**)&image );
      VT_FreeImage( &imres );
      VT_ErrorParse("unable to convert color image to grey one\n", 0);
    }
...
static int _VT_Grey2Color( vt_image *resIm, vt_image *theIm, vt_image *table )
#else
static int _VT_Grey2Color( resIm, theIm, table )
vt_image *resIm;
vt_image *theIm;
vt_image *table;
#endif
...
static int _VT_Color2Grey( vt_image *resIm, vt_image *theIm, vt_fpt *coeff )
#else
static int _VT_Color2Grey( resIm, theIm, coeff )
vt_image *resIm;
vt_image *theIm;
vt_fpt *coeff;
#endif
