...
int main( int argc, char *argv[] )
{
  ...
  imseed = _VT_Inrimage( par.names.in );
  if ( imseed == (vt_image*)NULL ) {
    VT_ErrorParse("unable to read input label image\n", 0);
  }
  ...
  if ( imgrad == (vt_image*)NULL ) {
    VT_FreeImage( imseed );
    VT_Free( (void**)&imseed );
    VT_ErrorParse("unable to read input gradient image\n", 0);
  }

  if ( imseed->dim.x != imgrad->dim.x || imseed->dim.y != imgrad->dim.y || 
       imseed->dim.z != imgrad->dim.z || imseed->dim.v != imgrad->dim.v ) {
    VT_FreeImage( imseed );
    VT_Free( (void**)&imseed );
    VT_FreeImage( imgrad );
    VT_Free( (void**)&imgrad );
  ...
  theDim[0] = imseed->dim.x;
  theDim[1] = imseed->dim.y;
  theDim[2] = imseed->dim.z;
  ...
  VT_Image( &imres );
  VT_InitFromImage( &imres, imseed, par.names.out, imseed->type );
  ...
  if ( watershed( imgrad->buf, imgrad->type,
		  imseed->buf, imres.buf, imseed->type, theDim ) != 1 ) {
    VT_FreeImage( imseed );
    VT_Free( (void**)&imseed );
    VT_FreeImage( imgrad );
    VT_Free( (void**)&imgrad );
    VT_FreeImage( &imres );
    VT_ErrorParse( "error when computing watershed\n", 0);
  }
