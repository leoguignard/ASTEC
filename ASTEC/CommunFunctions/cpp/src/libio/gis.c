...
int readGisHeader( const char* name,_image* im)
{
  ...
  status = sscanf( str,"%lu %lu %lu %d", &(im->xdim), &(im->ydim), 
		   &(im->zdim), &(im->vdim) );
  switch ( status ) {
  case 2 :    im->zdim = 1;
  case 3 :    im->vdim = 1;
...
int writeGisHeader( const _image* inr )
{
  ...
  /* dimensions
   */
  sprintf( str, "%lu %lu", inr->xdim, inr->ydim );
  if ( inr->vdim > 1 ) {
    sprintf( str+strlen(str), " %lu %d", inr->zdim, inr->vdim );
  }
  else if ( inr->zdim > 1 ) {
    sprintf( str+strlen(str), " %lu", inr->zdim );
  }
  sprintf( str+strlen(str), "\n" );
