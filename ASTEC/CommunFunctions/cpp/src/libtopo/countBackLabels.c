 ...
 * countBackLabels.c -
 *
 * $Id: countBackLabels.c,v 1.1 2000/07/26 17:34:36 greg Exp $
 *
 ...
 * 
 * CREATION DATE: 
 * Wed Jul 26 16:37:53 MET DST 2000
 *
 * ADDITIONS, CHANGES
 *
 */
      ...
      case SSHORT :
	{
	  ...
	  if ( imz == (vt_image*)NULL ) {
	    ...
	    for ( x=0; x<image->dim.x; x++ ) {
	      ...
	      resBuf[z][y+tY[z][y][x]][x+tX[z][y][x]] ++;
	    }
	  } else {
	    tZ = (s16***)imz->array;
	    for ( z=0; z<image->dim.z; z++ )
	    ...
	    for ( x=0; x<image->dim.x; x++ ) {
	      if ( theBuf[z][y][x] == 0 ) continue;
	      resBuf[z+tZ[z][y][x]][y+tY[z][y][x]][x+tX[z][y][x]] ++;
	    }
	  }
