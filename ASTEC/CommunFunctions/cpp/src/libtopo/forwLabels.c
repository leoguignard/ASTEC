 ...
 * forwLabels.c -
 *
 * $Id: forwLabels.c,v 1.1 2000/07/26 12:23:43 greg Exp $
 *
 ...
 * 
 * CREATION DATE: 
 * Wed Jul 26 11:48:04 MET DST 2000
 *
 * ADDITIONS, CHANGES
 *
 */
  ...
  case UCHAR :
      ...
      case SSHORT :
	{
	  ...
	  if ( imz == (vt_image*)NULL ) {
	    ...
	    for ( y=0; y<image->dim.y; y++ )
	    for ( x=0; x<image->dim.x; x++ ) {
	      if ( resBuf[z][y][x] != 0 ) continue;
	      if ( theBuf[z][y+tY[z][y][x]][x+tX[z][y][x]] == 0 ) continue;
	      resBuf[z][y][x] = theBuf[z][y+tY[z][y][x]][x+tX[z][y][x]];
	    }
	  } else {
	    tZ = (s16***)imz->array;
	    for ( z=0; z<image->dim.z; z++ )
	    ...
	    for ( x=0; x<image->dim.x; x++ ) {
	      if ( resBuf[z][y][x] != 0 ) continue;
	      if ( theBuf[z+tZ[z][y][x]][y+tY[z][y][x]][x+tX[z][y][x]] == 0 ) continue;
	      resBuf[z][y][x] = theBuf[z+tZ[z][y][x]][y+tY[z][y][x]][x+tX[z][y][x]];
	    }
	  }
