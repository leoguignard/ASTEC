...
\t -[niter|nangles] %d : nombre d'iterations\n\t si cette option n'est pas presente, on fait une seule itération\n\
\t -nsticks %d : nombre d'angles pour discrétiser le plan normal au champ de tenseur plate\n\
...
int main( int argc, char *argv[] )
{
  ...
  if (par.test == 1)
  {
    ...
    if (MT_Compute3DTestFields( &tfields, dimFields,
          angles, Nangles, par.r, par.alpha) != 1)
    {
      for (n=0;n<Nangles;n++){
        free(angles[n]);
        angles[n]=NULL;
  ...
  if (MT_Compute3DPlateFields(&pfields, dimFields, theCoeff,
