...
static char *usage = "[-label-in %s] [-label-ext %s]\n\
\t [-gradient-in %s] [-gradient-ext %s] [-delta %lf]\n\
\t [-pairs|pair|p] %d %d [...] | %s]\n\
\t [-p-in %f %f %f %f] [-p-ext %f %f %f %f] \n\
\t [-rigid | -affine] \n\
...
static char *detail = "\
\t -label-[in|ext] %s : nom des images de labels in|ext\n\
\t -gradient-[in|ext] %s : nom des images de gradients in|ext (barycentre des labels si non specifie)\n\
\t -delta %lf : ignore les labels d'une distance > au parametre\n\
\t -pairs %d %d [...] | %s : associe les paires de labels specifiees (in-ext [in-ext [...]])\n\
\t -p-in %f %f %f %f : equation du plan de l'image-in (coordonnees reelles)\n\
\t -p-ext %f %f %f %f : equation du plan de l'image-ext (coordonnees reelles)\n\
\t -trsf %s : fichier dans lequel est enregistree la transformation T_ext<-in calculee\n\
...
int main( int argc, char *argv[] )
{
  ...
  double r, rmin, *residus;
  double sd, sdmin, *std_deviations;
  int *pairs[2];
  int npairs;
  ...
  //if (par.npairs==0){
  //  if ((par.name_gradients.in[0] == '\0') || (par.name_gradients.ext[0] == '\0' ))
  //    MT_ErrorParse("missing input gradient images\n", 0);
  //}

  /*--- lecture des images labels d'entree ---*/
  images_labels[0] = _VT_Inrimage( par.name_labels.in );
  if ( images_labels[0] == (vt_image*)NULL )
    MT_ErrorParse("unable to read input image labels in\n", 0);
  ...
  if (VT_AllocImageBarycentresWithImage(&b_in, *images_labels[0]) != 1) {
      VT_FreeImage( images_labels[0] );
      VT_Free( (void**)&images_labels[0]);
      MT_ErrorParse("unable to allocate barycentre image for label-in\n", 0);
      return(0);
  }

  if(VT_ComputeLabelBarycentres(*images_labels[0], &b_in) != 1)
  {
      VT_FreeImage( images_labels[0] );
      VT_Free( (void**)&images_labels[0]);
      VT_FreeImageBarycentres(&b_in);
      MT_ErrorParse("unable to compute barycentre image for label-in\n", 0);
      return(0);
  }
  ...
  if ( images_labels[1] == (vt_image*)NULL ) {
    MT_ErrorParse("unable to read input image labels ext\n", 0);
  }
  ...
  if (VT_AllocImageBarycentresWithImage(&b_ext, *images_labels[1]) != 1) {
  ...
  if(VT_ComputeLabelBarycentres(*images_labels[1], &b_ext) != 1)
  {
      VT_FreeImage( images_labels[1] );
      VT_Free( (void**)&images_labels[1]);
      VT_FreeImageBarycentres(&b_in);
      VT_FreeImageBarycentres(&b_ext);
      MT_ErrorParse("unable to compute barycentre image for label-ext\n", 0);
      return(0);
  }
  ...
  if(_VT_VERBOSE_) {
      vt_barycentre theBary;
      fprintf(stdout, "label-ext : %s\n", par.name_labels.ext);
      fprintf(stdout, "\t- Number of labels : %d\n", b_ext.n);
      fprintf(stdout, "\t- Voxel size : %f %f %f\n", b_ext.vx, b_ext.vy, b_ext.vz);
      if(_verbose_>4 && b_ext.n>0)
      {
  ...
  if(par.npairs==0) {
      ...
      if (par.name_gradients.in[0]!='\0') {
        ...
        if ( images_gradients[0] == (vt_image*)NULL )
          MT_ErrorParse("unable to read input image gradient in\n", 0);
        ...
        if (VT_Barycentre(images_gradients[0], G_in) != 1) {
          ...
          MT_ErrorParse("unable to compute gradient-in barycentre\n", 0);
        }
      ...
      if(par.name_gradients.ext[0]!='\0') {
        ...
        if ( images_gradients[1] == (vt_image*)NULL ) {
          MT_ErrorParse("unable to read input image gradient ext\n", 0);
        }
        ...
        if(VT_Barycentre(images_gradients[1], G_ext) != 1) {
          ...
          MT_ErrorParse("unable to compute gradient-ext barycentre\n", 0);
        }
      ...
      if (_VT_VERBOSE_) {
        fprintf(stdout, "Barycentre_in  = [ %f   %f   %f ]\n", G_in[0],G_in[1],G_in[2]);
        fprintf(stdout, "Barycentre_ext = [ %f   %f   %f ]\n", G_ext[0],G_ext[1],G_ext[2]);
      }
      ...
      if (_VT_VERBOSE_) {
        fprintf(stdout, "Barycentre_in projected  = [ %f   %f   %f ]\n", G_in[0],G_in[1],G_in[2]);
        fprintf(stdout, "Barycentre_ext projected = [ %f   %f   %f ]\n", G_ext[0],G_ext[1],G_ext[2]);
      }
      ...
      if (1 && _verbose_>3) {
        fprintf(stdout, "#Transformation matrix (sym<->ext):\n");
      ...
      if (1 && _verbose_>3) {
        fprintf(stdout, "#Transformation matrix (ext<-theta):\n");
      ...
      if (1 && _verbose_>3) {
        fprintf(stdout, "#Transformation matrix (theta<-ext):\n");
      ...
      thetas=malloc(2*n*sizeof(double));
      ...
      residus=malloc(2*n*sizeof(double));
      if (residus==(double*)NULL) {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_in_trsf);
          VT_FreeImageBarycentres(&b_ext);
          VT_FreeImageBarycentres(&b_ext_trsf);
          free(thetas); thetas=NULL;
          MT_ErrorParse("unable to allocate thetas vector", 0);
      }
      std_deviations=malloc(2*n*sizeof(double));
      ...
      for (i=0 ; i<2*n ; i++) {
        ...
        if (1 && _verbose_>4) {
          fprintf(stdout, "#Transformation matrix (in<-theta = %f * PI):\n", theta/PI);
        ...
        if (1 && _verbose_>4) {
          fprintf(stdout, "#Transformation matrix (in<-ext, theta = %f * PI):\n", theta/PI);
        ...
        if (VT_TransformImageBarycentres(T_in_theta_ext_sym, b_ext, &b_ext_trsf) != 1) {
            ...
            VT_FreeImageBarycentres(&b_ext_trsf);
            free(residus); residus=NULL;
            free(std_deviations); std_deviations=NULL;
            free(thetas);  thetas=NULL;
        ...
        if( VT_ComputePointTrsf(pairs, npairs, b_in, b_ext, &r, &sd, T_tmp, par.flag_affine, par.estimator) != 1) {
            ...
            free(std_deviations); std_deviations=NULL;
            free(thetas);  thetas=NULL;
            free(pairs[0]); pairs[0]=NULL;
            free(pairs[1]); pairs[1]=NULL;
        ...
        if (VT_TransformImageBarycentres(T_tmp, b_ext, &b_ext_trsf) != 1) {
            VT_FreeImageBarycentres(&b_in);
            VT_FreeImageBarycentres(&b_in_trsf);
            VT_FreeImageBarycentres(&b_ext);
            VT_FreeImageBarycentres(&b_ext_trsf);
        ...
        if (VT_TransformImageBarycentres(T_tmp, b_in, &b_in_trsf) != 1) {
            VT_FreeImageBarycentres(&b_in);
            VT_FreeImageBarycentres(&b_in_trsf);
            VT_FreeImageBarycentres(&b_ext);
            VT_FreeImageBarycentres(&b_ext_trsf);
    ...
    //            fprintf(stdout, "#Transformation matrix (coordonnees de ext vers theta = %f * PI):\n", theta/PI);
                ...
                fprintf(stdout, "#Transformation matrix (in<-ext_S, theta = %f * PI ):\n", theta/PI);
              }
            ...
            if (_verbose_>3) {
              if (i%2==0) {
    //            fprintf(stdout, "#Transformation matrix (coordonnees de ext vers theta = %f * PI):\n", theta/PI);
                fprintf(stdout, "#Transformation matrix (ext<-in, theta = %f * PI ):\n", theta/PI);
              }
              else {
                fprintf(stdout, "#Transformation matrix (ext_S<-in, theta = %f * PI ):\n", theta/PI);
              }
        ...
        if((((r>=0 && r<rmin) || rmin<0 ) && 1)||0) {
            ...
            if(par.name_res.in[0] != '\0')
            {
                ...
                fprintf(fichier, "#Liste d'appariements (in-ext) trouvés:\n#\n");
                vt_barycentre theB_in;
                vt_barycentre theB_ext;
                for (j=0; j<npairs; j++) {
                    theB_in=b_in.barycentres[pairs[0][j]];
      ...
      if (_verbose_>1) {
        fprintf(stdout, "#Transformation matrix min (in<-ext) before point Transformation computation:\n");
      ...
      if(_verbose_>2) {
          ...
          fprintf(stdout, "Appariements utilises pour le calcul de la transformation :\n");
          for (j=0; j<npairs  ; j++) {
            theB_in=b_in.barycentres[pairs[0][j]];
            theB_ext=b_ext.barycentres[pairs[1][j]];
            fprintf(stdout, "( %d  %d )\n", theB_in.label, theB_ext.label);
      ...
      pairs[0]=malloc(par.npairs*sizeof(int));
      if(pairs[0]==NULL) {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_in_trsf);
          VT_FreeImageBarycentres(&b_ext);
      ...
      pairs[1]=malloc(par.npairs*sizeof(int));
      if(pairs[1]==NULL) {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_in_trsf);
          VT_FreeImageBarycentres(&b_ext);
      ...
      if(par.name_res.in[0] != '\0')
      {
          ...
          fprintf(fichier, "#Liste d'appariements (in-ext) entrés :\n#\n");
          vt_barycentre theB_in;
          vt_barycentre theB_ext;
          for (i=0; i<par.npairs; i++) {
  ...
  if(_VT_VERBOSE_) {
      ...
      if(par.npairs==0) {
          ...
          if (imin%2==0) fprintf(stdout, "#Transformation matrix (ext<-in, obtained with theta = %f * PI ):\n", thetas[imin]/PI);
          //else fprintf(stdout, "#Transformation matrix (ext_R<-in, theta = %f * PI ):\n", thetas[imin]/PI);
      }
      else fprintf(stdout, "#Transformation matrix (ext<-in):\n");
  ...
  if(par.name_res.out[0] != '\0')
  {
      ...
      //vt_barycentre theB_in;
      //vt_barycentre theB_ext;
      //fprintf(fichier, "#Appariements:");
      //for (j=0; j<npairs  ; j++) {
      //    if(j%2==0) fprintf(fichier, "\n");
      //    theB_in=b_in.barycentres[pairs[0][j]];
      //    theB_ext=b_ext.barycentres[pairs[1][j]];
      //    fprintf(fichier, "# ( %d  %d ) \t", theB_in.label, theB_ext.label);
      //}
      fprintf(fichier, "\n");
      fclose(fichier);
  }
  ...
  if (par.name_res.ext[0] != '\0') {
      ...
      fprintf(fichier, "#Liste d'appariements (in-ext) recalcules :\n#\n");
      vt_barycentre theB_in;
      vt_barycentre theB_ext;
  ...
  if (par.npairs == 0 && par.dices_in == 1)
  {
      ...
      dices=malloc(npairs*sizeof(double));
      if(dices==NULL)
      {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_ext);
      ...
      int *labels[2];
      labels[0]=malloc(npairs*sizeof(int));
      if(dices==NULL)
      {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_ext);
      ...
      labels[1]=malloc(npairs*sizeof(int));
      if(dices==NULL)
      {
          VT_FreeImageBarycentres(&b_in);
          VT_FreeImageBarycentres(&b_ext);
      ...
      fprintf(fichier, "#Labels In \tLabels Ext \tDices\n");
      double mean_dices=0;
                      ...
                      local_par *par )
{
  ...
  while ( i < argc ) {
    if ( argv[i][0] == '-' ) {
      ...
      else if ( strcmp ( argv[i], "-gradient-in" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -gradient-in...\n",0);
        sprintf(par->name_gradients.in, "%s", argv[i]);
      }

      else if ( strcmp ( argv[i], "-gradient-ext" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -gradient-ext...\n",0);
        sprintf(par->name_gradients.ext, "%s", argv[i]);
      }
      else if ( strcmp ( argv[i], "-label-in" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -label-in...\n",0);
        sprintf(par->name_labels.in, "%s", argv[i]);
      }
      else if ( strcmp ( argv[i], "-label-ext" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -label-ext...\n",0);
      ...
      else if ( strcmp ( argv[i], "-p-in-file" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -p-in-file...\n",0);
        sprintf(par->name_planes.in, "%s", argv[i]);
        par->flag_name_planes_in=1;
      }
      else if ( strcmp ( argv[i], "-p-ext-file" ) == 0 ){
        i += 1;
        if ( i>= argc ) MT_ErrorParse( "parsing -p-ext-file...\n",0);
        sprintf(par->name_planes.ext, "%s", argv[i]);
        par->flag_name_planes_ext=1;
      }

      else if ( strcmp ( argv[i], "-p-in" ) == 0 ) {
          i += 1;
          if ( i >= argc)    MT_ErrorParse( "parsing -p-in...\n", 0 );
          status = sscanf( argv[i],"%lf",&(par->p_in[0]) );
          if ( status <= 0 ) MT_ErrorParse( "parsing -p-in...\n", 0 );
          i += 1;
          if ( i >= argc)    MT_ErrorParse( "parsing -p-in...\n", 0 );
      ...
      else if ( strcmp ( argv[i], "-p-ext" ) == 0 ) {
          i += 1;
          if ( i >= argc)    MT_ErrorParse( "parsing -p-ext...\n", 0 );
          status = sscanf( argv[i],"%lf",&(par->p_ext[0]) );
          if ( status <= 0 ) MT_ErrorParse( "parsing -p-ext...\n", 0 );
          i += 1;
          if ( i >= argc)    MT_ErrorParse( "parsing -p-ext...\n", 0 );
