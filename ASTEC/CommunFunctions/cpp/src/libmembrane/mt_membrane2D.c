			    ...
			    enumStructureColor color ) 
{
  ...
  for ( s = 0; s < nbs; s ++ ) {
    ...
    if ( MT_Filter2Dimages( theIm, &theIms, theCoeff ) != 1 ) {
    ...
    MT_Compute2DResponse( &theIms, color, tau, scale );
...
void MT_Compute2DExtrema( vt_3Dimres *imsRes,
			  vt_image *imExt )
{
...
void MT_Compute2DResponse( vt_2Dimages *par, enumStructureColor color,
...
int MT_Filter2Dimages( vt_image *im, vt_2Dimages *par, float *theCoeffs )
{
  char *proc = "MT_Filter2Dimages";
...
///////////////////////// TENSOR VOTING 2D ////////////////////////////////////
// rand_(a) retourne une valeur entière aléatoire comprise entre 0 et a-1
int rand_(int a){
    return(rand()%a);
}
...
int MT_Compute2DTensorVoting(mt_2Dtensor *theTensor, vt_image **imBin, double scale, int Niter,
        ...
        char *parName, int writeImages)
{
  ...
  case UCHAR :
    ...
    bin=malloc(nslice*sizeof(float**));
    for (slice=0;slice<nslice;slice++)
    {
      bin[slice]=malloc(dimy*sizeof(float*));
      for (y=0;y<dimy;y++)
      {
        bin[slice][y]=malloc(dimx*sizeof(float));
        for (x=0;x<dimx;x++)
  ...
  /* Calcul des angles répartis de façon homogène sur le cercle unite */
  if (_verbose_)
    fprintf(stdout, "%s: computating angles...\n", proc);

  Nangles = MT_Compute2DAngles(&angles, nangles);
  if (Nangles <= 0) {
    if (_verbose_)
  ...
  if (initHessian != NONE)
    MT_Init2DTensorFromAngle(theTensor);
  ...
  for (i=0;i<=Niter;i++)
  {
    ...
    MT_Reinit2DTensorImage(theTensor);
    ...
    for (slice=0;slice<nslice;slice++)
    {
      ...
      for (x=0;x<dimx;x++)
      {
        ...
        if (lambda2-lambda1 > FLTZERO)  // STICKS
        {
          ...
          n = nearest2DAngle(theta, angles, Nangles);
          MT_Add2DField(theTensor, sfields[n], lambda2-lambda1,x,y,slice, typeVote);
        }

        if (lambda1 > FLTZERO)          // BALLS
          MT_Add2DField(theTensor, bfield, lambda1,x,y,slice, typeVote);
      }
    }
    ...
    for (x=0;x<dimx;x++)
    {
      ...
      if (set2DTensor(theTensor, x,y,slice, typeVote) !=1)
      {
        if (_verbose_)
...
void MT_Reinit2DTensorImage(mt_2Dtensor *theTensor)
{
...
void MT_Init2DTensorFromAngle(mt_2Dtensor *theTensor)
{
  int x,y,z;
  int dimx = theTensor->imxx.dim.x;
  int dimy = theTensor->imxx.dim.y;
          ...
          double *angles, double *theCoeffs, int Nangles, enumTVmode mode)
{
  ...
  (*sfields) =  malloc(Nangles*sizeof(mt_2Dtensor));
  ...
  for (n=0; n<Nangles; n++) {
    angle=angles[n];
    field = (*sfields+n);
    if (MT_Compute2DStickField(field, dimFields, angle, theCoeffs, mode) != 1)
    {
      if (_verbose_)
...
int MT_Compute2DStickField( mt_2Dtensor *sfield, int *dimFields,
          double angle, double *theCoeffs, enumTVmode mode)
{
  ...
  for (x=0;x<dimFields[0];x++) {
    ...
    /* Calcul du facteur de décroissance en le pixel [x,y] */
...
int MT_Add2DField(mt_2Dtensor *tensorImg, mt_2Dtensor field, double coef,
...
int set2DTensor(mt_2Dtensor *tensorImg, int x, int y, int slice,
          enumVote mode)
{
  char *proc = "set2DTensor";
  float ***theXX = (float***)(tensorImg->imxx.array);
  float ***theYY = (float***)(tensorImg->imyy.array);
  float ***theXY = (float***)(tensorImg->imxy.array);
...
int nearest2DAngle(double theta, double *angles, int Nangles)
{
  int n, N=0;
  double dot, DOT=0;
...
int MT_Compute2DAngles(double **angles, int Nangles)
{
...
void VT_Write2DTensorWithName( mt_2Dtensor *par, char *genericname)
{
  char name[256];
  sprintf( name, "%s.imxx.inr", genericname );
