...
\t -sigma %lf : ecart-type pour la ponderation selon l'ecart angulaire\n\
...
\t -dmin %d : e-t minimal pour la distance au plan pour le least square\n\
\t -p %lf : option de ponderation des points par une gaussienne d'e-t %lf \n\
\t          par rapport a l'ecart angulaire au plan de l'iteration precedante\n\
...
//  int type = (int)UCHAR;
  ...
  unsigned char ***theInU8;
  double n[4], m[3], nold[4];
  double sigma2x2;
  double coef, tht, phi, alpha;
  ...
  if (1 || par.plane == 1)
  {
	...
	if (par.init == 1) {
      H=malloc(sizeof(double)*ind_max);
	  if(H==(double*)NULL) 
	  {
		VT_FreeImage( &imres );
		if (par.plane == 1) 
  ...
  if(1)
  {
	if(flag_3D==1)
	{
      ...
      do
	  {
		...
		E=VT_MeasureSquareError(list, nb, n, L, par.sigma);
		...
		do
		{
		  ...
		  if (VT_totalLeastSquarePlane(list,nb, L, par.sigma_p, n, &E) != 1)
		  {
			VT_FreeImage( imageIn );
			if (par.sigma_p > 0) { 
			  VT_FreeImage( imageTht  );
                      ...
                      local_par *par )
{
  ...
  while ( i < argc ) {
    if ( argv[i][0] == '-' ) {
      ...
      else if ( strcmp ( argv[i], "-voxel-in" ) == 0 || strcmp ( argv[i], "-vox-in" ) == 0 ) {
        par->realIn = 0;
      }
