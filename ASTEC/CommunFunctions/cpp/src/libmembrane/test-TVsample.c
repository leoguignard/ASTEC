...
static int SampleBinRegular(vt_image *theIm, double sample, int *thePt, double *theNormal, double *ray);
    ...
    if (par.sample > 0 && par.sample < 1)
    {
      ...
      case _REGULAR_ :
        ...
        if(theNormal[0]==theNormal[1] && theNormal[0]==0 && theNormal[2]==1)
        {
          ...
          if (SampleBinRegular(theIm, par.sample, thePt, theNormal, &ray) != 1 )
          {
            if (par.initHessian == 0)
            {
              VT_FreeImage( theTht );
