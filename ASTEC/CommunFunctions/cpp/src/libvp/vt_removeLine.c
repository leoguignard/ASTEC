...
int VP_RemoveLines( vt_image *theIm, vt_image *resIm, double coefficient )
{
  ...
  for ( y=0; y<theIm->dim.y; y++ ) {
    ...
    ymin = ymax = y;
    while ( slist.data[ymax].flag && ymax < theIm->dim.y-1 ) ymax ++;
    if ( ymax < theIm->dim.y-1 ) {
      ymax --;
    }
    else if ( ymax == theIm->dim.y-1 && ! slist.data[ymax].flag ) {
      ymax --;
    }
